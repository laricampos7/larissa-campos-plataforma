# -*- coding: utf-8 -*-
"""Story/Feed mostrando a tela principal do app (Inicio), com nome da aluna borrado."""
import os
...
SRC = r"C:\Users\Larissa\Downloads\WhatsApp Image 2026-06-23 at 16.55.04 (2).jpeg"

def prep_screen():
    im = Image.open(SRC).convert("RGB")
    # corta a parte de baixo bagunçada (barra do navegador + botao de check-in)
    im = im.crop((0, 0, 739, 1290))
    # borra nome e avatar da aluna (privacidade)
    for box in [(118, 358, 362, 412), (38, 360, 118, 438)]:
        reg_im = im.crop(box).filter(ImageFilter.GaussianBlur(16))
        im.paste(reg_im, box)
    return im
...
def build(W, H, glow_lines, head_y, head_size, f, out_name):
    ...
    center(d, W, head_y[1], "TREINO 100%", black(head_size), WHITE)
    center(d, W, head_y[2], "PERSONALIZADO", black(head_size), BRAND)
    place(canvas, SCREEN, f["top"], f["bottom"], int(W*0.86))
    if f.get("cta"):
        center(d, W, f["cta_y"], "Um app só seu, com o seu treino", bold(34), WHITE)
...
build(1080, 1920, 260, (118, 172, 245), 66,
      {"top": 360, "bottom": 1560, "cta": True, "cta_y": 1600, "btn_y": 1670, "handle_y": 1800, "cref_y": 1856},
      "STORY - Tela do App.jpg")

build(1080, 1350, 190, (66, 116, 184), 54,
      {"top": 260, "bottom": 1175, "cta": False, "handle_y": 1245, "cref_y": 1296},
      "POST FEED - Tela do App.jpg")
